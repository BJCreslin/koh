...
public class ExcelInputPanel implements PaneInterface {
    ...
    private void createChangelogMigration(JFrame parent) {
        try {
            if (!validateFile(file, parent)) {
                return;
            }
    ...
    private JButton createModernFileButton(JFrame frame) {
        ImageIcon icon = null;
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("images.png")) {
            if (is != null) {
                BufferedImage img = ImageIO.read(is);
                Image scaledImage = img.getScaledInstance(24, 24, Image.SCALE_SMOOTH);
                icon = new ImageIcon(scaledImage);
            }
        } catch (IOException e) {
            logger.debug("Не удалось загрузить иконку", e);
        }
        ...
        fileButton.addActionListener(e -> {
            ...
            try {
                ...
                if (option == JFileChooser.APPROVE_OPTION) {
                    ...
                    createChangelogMigration(frame);
     ...
     * @param parent родительское окно для отображения сообщений
     * @return true, если файл валиден
     */
    private boolean validateFile(File file, JFrame parent) {
        if (file == null) {
            JOptionPane.showMessageDialog(parent, "Файл не выбран.", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (!file.exists()) {
            JOptionPane.showMessageDialog(parent, "Файл не существует.", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (!file.canRead()) {
            JOptionPane.showMessageDialog(parent, "Нет доступа к файлу.", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        if (file.length() == 0) {
            JOptionPane.showMessageDialog(parent, "Файл пустой.", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        return true;
    }
